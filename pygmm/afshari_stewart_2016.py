...
class AfshariStewart2016(model.Model):
    ...
    def __init__(self, scenario):
        ...
        stress_drop = np.exp(  b_1 + b_2 * (np.minimum(s.mag, mag_2)  - mag_star) + b_3 * np.maximum((s.mag - mag_2),0) )
        #corner frequency
        f_0 = np.array([self.corner_freq(s.mag, s_d) for s_d in stress_drop])
        #source duration
        ...
        R_2 = 50
        #path duration
        F_P = c_1 * np.minimum(s.dist_rup, R_1) + c_2 * np.maximum((np.minimum(s.dist_rup, R_2) - R_1),0) + c_3 * np.maximum((s.dist_rup - R_2),0)
        ...
        tau = tau_1 + (tau_2 - tau_1) * (np.minimum(np.maximum(s.mag,6.5), 7.0)  - 6.5) / (7.0 - 6.5) 
        phi = phi_1 + (phi_2 - phi_1) * (np.minimum(np.maximum(s.mag,5.5), 5.75) - 5.5) / (5.75 - 5.50)
    ...
    @property
    def std_err(self):
        return self._std_err[:3]

    def corner_freq(self, mag, stress_drop):
