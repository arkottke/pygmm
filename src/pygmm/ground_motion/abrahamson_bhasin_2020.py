...
@register(provides=("pgv",), input="conditional")
class AbrahamsonBhasin2020(model.Model):
    ...
    def __init__(
        ...
        if psa is not None:
            ...
            method = "pga"
            ln_gm = np.log(pga)
        elif psa_1s is not None:
            method = "psa(T=1s)"
            ln_gm = np.log(psa_1s)
        else:
            raise ValueError(
                "One conditioning intensity measure is required: pass pga=, "
                "psa_1s=, or psa=."
            )
        ...
        self._ln_pgv = ln_mean
        self._tau = tau
        self._phi = phi
        self._ln_std = np.sqrt(phi**2 + tau**2)
    ...
    @property
    def ln_pgv(self):
        """Natural log of the median PGV (cm/sec)."""
        return self._ln_pgv

    @property
    def pgv(self):
        """Median PGV (cm/sec)."""
        return np.exp(self._ln_pgv)

    @property
    def ln_std(self):
        """Total lognormal standard deviation of PGV."""
        return self._ln_std
    ...
    @property
    def pgv_plus_sigma(self):
        """84th-percentile PGV (cm/sec): ``exp(ln_pgv + ln_std)``."""
        return np.exp(self._ln_pgv + self._ln_std)

    @property
    def pgv_minus_sigma(self):
        """16th-percentile PGV (cm/sec): ``exp(ln_pgv - ln_std)``."""
        return np.exp(self._ln_pgv - self._ln_std)
