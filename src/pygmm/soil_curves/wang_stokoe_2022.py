...
def _to_decimal(*keys):
    """Divide the named kwargs by 100 (percent → decimal) before calling the function."""

    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            return func(*args, **{k: v / 100 if k in keys else v for k, v in kwargs.items()})
    ...
    **kwds
    ...
    FACTORS = {
        "clean_sand_and_gravel": [
            "stress_mean", "fines_cont", "coef_unif", "diam_50", "water_cont", "void_ratio", "diam_50",
        ],
        "nonplastic_silty_sand": ["stress_mean", "void_ratio", "fines_cont", "water_cont"],
        "clayey_soil": ["stress_mean", "void_ratio", "plas_index", "fines_cont", "ocr", "water_cont"],
    }
    ...
    def __init__(
        ...
        **kwds,
        ...
        if soil_group not in self.LEVELS:
            raise ValueError(f"Unknown soil_group {soil_group!r}. Choose from {list(self.LEVELS)}")
        self._index_params = {
            p: kwds[p]
    ...
    def _create_name(self) -> str:
        fmt = {
            "coef_unif": "Cᵤ={:.1f}", "diam_50": "D₅₀={:.1f} mm",
            "fines_cont": "FC={:.0f} %", "ocr": "OCR={:.1f}",
            "plas_index": "PI={:.0f}", "stress_mean": "σₘ'={:.1f} kN/m²",
            "void_ratio": "e={:0.2f}", "water_cont": "w_c={:.1f}%",
        }
    ...
    def calc_mod_reduc(cls, strains: npt.ArrayLike, soil_group: str, **kwds) -> np.ndarray:
        ...
        if soil_group == "clean_sand_and_gravel":
            if level == 0:
                ...
                a, b = 0.804, 0.882
                gamma_mr = (0.13 * kwds["void_ratio"] ** 0.545 - 0.043) * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.45
            elif level == 2:
                a, b = 0.834, 0.839
                gamma_mr = (0.05 * kwds["void_ratio"] ** (0.1 * kwds["coef_unif"]) + 0.011) * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.45
            else:
                a = 0.834 + kwds["fines_cont"]
                b = 0.844 - 1.897 * kwds["fines_cont"]
                gamma_mr = (0.048 * kwds["void_ratio"] ** (0.089 * kwds["coef_unif"]) + 0.008) * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.4
            ...
            if level == 0:
                ...
                gamma_mr = (0.029 * kwds["void_ratio"] - 0.003) * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.335
            else:
                a = (1.495 * kwds["void_ratio"] + 3.079 * kwds["fines_cont"]) ** 0.121
                b = 0.486 - 0.006 * kwds["stress_mean"] * _KPA_TO_ATM
                gamma_mr = (0.031 * kwds["void_ratio"] - 0.003) * (kwds["stress_mean"] * _KPA_TO_ATM) ** (0.405 - 0.193 * kwds["fines_cont"])
            ...
            if level == 0:
                ...
                a, b = 1.185, 0.475
                gamma_mr = 0.035 * kwds["void_ratio"] * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.276
            elif level == 2:
                a = 0.966 + 0.378 * kwds["fines_cont"]
                b = 0.596 - 0.207 * kwds["fines_cont"]
                gamma_mr = (0.031 * kwds["void_ratio"] + 0.004 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** 0.25
            elif level == 3:
                a = 0.972 + 0.419 * kwds["fines_cont"]
                b = 0.571 - 0.2 * kwds["fines_cont"]
                gamma_mr = (0.025 * kwds["void_ratio"] + 0.0015 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM + 0.375 * kwds["ocr"]) ** 0.358
            else:
                a = 0.896 + 0.412 * kwds["fines_cont"] + 0.534 * kwds["plas_index"]
                b = 0.586 - 0.098 * kwds["void_ratio"] - 0.135 * kwds["fines_cont"]
                gamma_mr = (0.02 * kwds["void_ratio"] + 0.004 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM + 0.42 * kwds["ocr"]) ** (0.447 - 0.27 * kwds["plas_index"])
        else:
    ...
    def calc_damping_min(cls, soil_group: str, **kwds) -> float:
        ...
        if soil_group == "clean_sand_and_gravel":
            if level == 0:
                ...
                d_min = 0.55 * (1 + 29.03 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.13
            elif level == 2:
                d_min = 0.64 * (0.26 - kwds["water_cont"]) ** 0.11 * (1 + 32.8 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.12
            elif level == 3:
                d_min = 0.55 * (1 - kwds["water_cont"]) ** (-12.49 + 19.45 * kwds["void_ratio"]) * (1 + 23.44 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.14
            else:
                d_min = 0.6 * (0.99 + kwds["water_cont"]) ** (7.45 - 15.23 * kwds["void_ratio"] + 4.29 * kwds["diam_50"]) * (1 + 21.17 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.14
        elif soil_group == "nonplastic_silty_sand":
            if level == 0:
                d_min = 1.47 * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.2
            elif level == 1:
                d_min = 39.11 * (0.44 * kwds["void_ratio"]) ** (4.32 * kwds["void_ratio"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.19
            else:
                d_min = 52.16 * (0.41 * kwds["void_ratio"]) ** (0.81 * kwds["fines_cont"] + 5.2 * kwds["void_ratio"]) * (1 + 5.35 * kwds["fines_cont"]) * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.19
        elif soil_group == "clayey_soil":
            if level == 0:
                d_min = 2.55 * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.11
            elif level == 1:
                d_min = 7.62 * 13.48 ** -kwds["void_ratio"] * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.29 + 0.72 ** -kwds["void_ratio"]
            elif level == 2:
                d_min = (
                    7.29 * 8 ** (-kwds["void_ratio"] - 3.31 * kwds["plas_index"])
                    * (1 + 148 * kwds["plas_index"] ** 1.95)
                    * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.2
                    + (0.5 * kwds["plas_index"]) ** (2.54 - 1.8 * kwds["void_ratio"])
                )
            else:
                d_min = (
                    4.86 * (1.99 + kwds["fines_cont"]) ** (-1.91 * kwds["void_ratio"] - 6.5 * kwds["plas_index"])
                    * (1 + 106.75 * kwds["plas_index"] ** 1.64)
                    * (kwds["stress_mean"] * _KPA_TO_ATM) ** -0.19
                    + (0.46 * kwds["plas_index"]) ** (1.73 - 1.34 * kwds["void_ratio"])
                )
        else:
            raise ValueError("Invalid soil group")
        return d_min / 100
    ...
    def calc_damping(cls, strains, soil_group, damping_min=None, **kwds) -> np.ndarray:
        ...
        if soil_group == "clean_sand_and_gravel":
            if level == 0:
                ...
                d = 21.17
                gamma_d = 0.13 * (kwds["stress_mean"] * _KPA_TO_ATM + 17.94 * kwds["fines_cont"]) ** (0.45 - kwds["fines_cont"])
            else:
                c = 0.93 * np.exp(0.34 - 0.8 * kwds["void_ratio"])
                d = 18.13
                gamma_d = 0.13 * kwds["coef_unif"] ** -0.31 * (kwds["stress_mean"] * _KPA_TO_ATM + 22.04 * kwds["fines_cont"]) ** (0.47 - kwds["fines_cont"])
        elif soil_group == "nonplastic_silty_sand":
            if level == 0:
                c, d = 1.187, 13.125
                ...
                d = 12.09
                gamma_d = 0.0066 * (kwds["stress_mean"] * _KPA_TO_ATM + 5.79 * kwds["void_ratio"]) ** 1.01
            else:
                c = 1.39 * np.exp(0.27 * kwds["void_ratio"])
                d = 12.13
                gamma_d = 0.0025 * (kwds["stress_mean"] * _KPA_TO_ATM + 5.73 * kwds["void_ratio"] + 9.17 * kwds["fines_cont"]) ** (1.47 - 0.52 * kwds["fines_cont"])
        elif soil_group == "clayey_soil":
            if level == 0:
                c, d = 1.12, 19.47
                ...
                c, d = 1.36, 15.16
                gamma_d = 0.29 * (0.017 * kwds["stress_mean"] * _KPA_TO_ATM + kwds["water_cont"]) ** (1.15 + kwds["water_cont"])
            elif level == 2:
                c = 1.48 ** (0.53 + kwds["plas_index"])
                d = 15.61
                gamma_d = 0.07 * (0.06 * kwds["stress_mean"] * _KPA_TO_ATM + 2.69 * kwds["water_cont"]) ** (1.06 + kwds["water_cont"] - kwds["plas_index"])
            else:
                c = (1.91 * kwds["fines_cont"]) ** (1.62 * kwds["plas_index"])
                d = 21.7
                gamma_d = 0.11 * (0.12 * kwds["stress_mean"] * _KPA_TO_ATM + 5.29 * kwds["water_cont"] - kwds["fines_cont"]) ** (1.45 - kwds["plas_index"] + kwds["water_cont"] - 1.09 * kwds["fines_cont"])
        else:
            raise ValueError("Invalid soil group")
